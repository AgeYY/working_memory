...
class TrainStepper(object):
    ...
    def __init__(self, model, hp, is_cuda=True, device_id=0):
        ...
        self.out_act_fcn = self.rnn_net.out_act_fcn
        ...
        self.l2_jac = torch.tensor(hp['l2_jac'], device=self.device)
    ...
    def cost_fcn(self, **kwargs):
        ...
        self.firing_rate_binder = self.act_fcn(self.state_binder) + 1

        self.outputs = self.out_act_fcn(torch.matmul(self.firing_rate_binder, self.out_weight) + self.out_bias)
        ...
        self.cost_lsq = torch.mean(
            ...
            #torch.sum((torch.abs(self.outputs - target_outputs)) * cost_mask, dim=0) / cost_mask_length)

        # calculate cost_reg
        self.cost_reg = self._0
        self.cost_reg_jac = self._0
        ...
        time_len = self.state_binder.size()[0];
        batch_size = self.state_binder.size()[1];

        jreg = JReg(n=self.hp['num_proj'])
        if self.l2_jac > 0:
            temp = self._0
            for i in range(cost_start_time + 1, cost_end_time): # Calculate the jacobian of every time points. Skipp the initial time since it has no grad
                if self.state_collector[i].requires_grad:
                    self.cost_reg_jac = self.cost_reg_jac + jreg(self.state_collector[i], self.state_collector[i+1] - self.state_collector[i]) * self.l2_jac
                    temp = temp + self.cost_reg_jac
            self.cost_reg = self.cost_reg + temp / (cost_end_time - cost_start_time - 1) / self.state_collector[0].size()[1] # devide time and the hidden size
